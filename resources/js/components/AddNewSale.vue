        ...
        <table class="table table-bordered">
            <thead class="bg-dark">
                <tr class="text-white text-center">
                    ...
                    <th scope="col" style="width: 150px;">Items </th>
            ...
            <tbody id="table-body">
                ...
                <tr class="text-center" v-for="(input, index) in inputs" :key="input.id">
                    ...
                    <!-- <td><vue-select @change="setSupplier($event, input.id)" :options="all_suppliers" name="supplier_id[]"
                            placeholder="Select Supplier" /></td> -->
                    <td><vue-select @change="setProductValue($event, input.id)" :options="all_products" name="product_id[]"
                            placeholder="Select Product" /></td>

                    <td v-if="type == 'Aluminium'">
                        <select name="color[]" id="" class="form-control" @change="selectColorSetRate($event, input.id)"
                            required>
                    ...
                    <td>
                        <input type="number" step="any" min="0" name="qty[]" :id="`qty` + input.id" class="form-control"
                            @keyup="updateInput($event, input.id)" @change="updateInput($event, input.id)"
        ...
        <div class="row mt-5">
            ...
            <div class="col-lg-3">
                <table class="table table-bordered">
                    ...
                    <tr>
                        <td>
                            <div class="d-flex">Customer Commision &nbsp; <span>
                                    <input @keyup="calculate_customer_comm($event)" name="customer_com_percent"
                                        id="customer_com_percent" type="text" style="width: 80px"
                                        placeholder="ex: 15%"></span></div>
                        </td>
                        <td><span class="font-weight-bold" id="customer_comm_show"></span></td>
                        <input type="text" hidden id="customer_grand_total" name="customer_comm_show">
                    </tr> -->
                    <!-- <tr>
                        <td>
                            <div class="d-flex">Customer Advance</div>
                        </td>
                        <td><span class="font-weight-bold"><input type="text" name="customer_advenced" style="width: 80px"
                    ...
                    <tr>
                        <td class="text-right">Grand Total</td>
                        <td><span class="font-weight-bold" id="grand_total"></span> TK</td>
                        <input type="text" name="grandTotal" hidden id="grand_total">
                    </tr>

                    <tr>
                        <td>
                            <div class="d-flex">Paid Amount</div>
                        </td>
                        <td>
                            <span class="font-weight-bold">
                                <input type="text" name="customer_advanced" id="paid_amount_input" style="width: 80px"
                                    placeholder="ex: 1000TK" @input="handleAdvane">
                            </span>
                        </td>
                    </tr>
                    ...
                    <tr>
                        <td>
                            <div class="d-flex">Due Amount</div>
                        </td>
                        <td>
                            <span> TK</span>
                            <span class="font-weight-bold" id="due_amount"></span>
...
export default {
    ...
    methods: {
        ...
        grandTotal() {
            ...
            var paidAmount = parseFloat(document.getElementById('paid_amount_input').value) || 0;
            var dueAmount = Math.max(parseFloat(total_value) - paidAmount, 0);

            var grand_total = document.getElementById('grand_total');
            grand_total.innerText = total_value;

            var dueAmountElement = document.getElementById('due_amount');
            dueAmountElement.innerText = dueAmount;
            document.getElementById('due_amount_input').value = dueAmount;
        },
